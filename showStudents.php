...
<body>
    ...
    if($result = $mysqli->query($sql)) {   // $mysqli - наш об'єкт, через який здійснюємо підключення, query - метод, який дозволяє виконати довільний запит
        ...
        printf("<table><tr><th>ID</th><th>ПІБ</th><th>ID групи</th></tr>");   // <br> в html - розрив рядка
        ...
        while( $row = $result->fetch_assoc() ){ // fetch_assoc() повертає рядок із запиту у вигляді асоціативного масиву, наприклад $row = ['id'=>'1', 'pib'=>'Олександр', 'grupa'=>'ІПЗ-31']
            printf("<tr><td>%s</td><td>%s</td><td>%s</td></tr>", $row['id'], $row['pib'], $row['group_id']); //виводимо результат на сторінку
        };
        printf("</table>");
        /*Звільняємо пам'ять*/
        $result->close();
    ...
    <ul>
        ...
        <li><a href="globalUpdateStudentsForm.php">Змінити ID групи всім учасникам цієї групи</a><br></li>
        <li><a href="globalDeleteStudentsForm.php">Видалити всіх студентів з однієї групи</a><br></li>
