...
<body>
    ...
    $sql = 'SELECT g.group_name, COUNT(s.group_id) AS "student_count" FROM groups AS g INNER JOIN students AS s ON g.id = s.group_id GROUP BY s.group_id';
    ...
    if($result = $mysqli->query($sql)) {
        ...
        while( $row = $result->fetch_assoc() ){
            printf("<tr><td>%s</td><td>%s</td></tr>", $row['group_name'], $row['student_count']);
        };
        printf("</table>");
        $result->close();
    };
